...
import cs355.view.DrawShape;
import cs355.view.MyAffineTransform;

public class HitTest {

	public static int lineHitTest(Line line, Point2D.Double e) {
		
		int bigDiameter = DrawShape.bigDiameter;
		...
		MyAffineTransform.rotate(worldToObj,-line.getRotation());
		...
		double y = translateHitPoint.getY();
			...
			if(Math.abs(x) < bigDiameter/2 && Math.abs(y) < bigDiameter/2) {
			
			double hitDistance = Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2));
			
			if(hitDistance < bigDiameter/2) {
	...
	public static boolean handleHitTest(Shape shape, Point2D.Double e) {
		...
		MyAffineTransform.rotate(worldToObj,-shape.getRotation());
		...
		offset += 4/DrawShape.zoomFactor+bigDiameter/2;
		offset = -offset;
		y = y - offset;
		...
		if(absX < bigDiameter/2 && absY < bigDiameter/2) {
	...
	public static boolean hitTest(Shape shape, Point2D.Double e) {
		...
		if(shape instanceof Line) {
			...
			if(qDistance <= 4/DrawShape.zoomFactor) {
